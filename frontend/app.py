...
with st.sidebar:
    ...
    st.divider()
    num_beams = st.slider("Beam width", 1, 8, 8)

# ---------------------------------------------------------------------------
# Main area — question input inside a form (no reruns while typing)
# ---------------------------------------------------------------------------
    ...
    if data["executed"]:
        if data["results"]:
            ...
            if data.get("truncated"):
                st.caption(f"Results truncated — showing first 10,000 rows.")
        else:
